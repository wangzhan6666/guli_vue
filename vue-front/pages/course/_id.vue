...
export default {
  ...
  methods: {
    ...
    initComment() {
      ...
      commentApi.getCommentInfo(this.page, this.limit, this.courseId)
        .then(response => {
    ...
    addComment() {
      ...
      commentApi.addCommentInfo(this.comment)
        .then(response => {
          //提示注册成功
          this.$message({
            type: 'success',
    ...
    gotoPage(page){
      commentApi.getCommentInfo(page, this.limit, this.courseId)
        .then(response => {
            this.data = response.data.data
        })
      }
